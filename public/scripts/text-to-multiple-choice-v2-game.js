...
function handleKeyPress(event) {
  ...
  if (selectedOption === gameState.correctOptionIndex) {
    ...
    handlePenalty(selectedOption, gameState.correctOptionIndex);
  }
}
...
function startCountdown() {
  ...
  const countInterval = setInterval(() => {
    ...
    if (count === 0) {
      ...
      currentText.style.display = "grid";
      startRound();
    } else {
      countdown.textContent = count;
    }
...
function showCurrentQuestion() {
  ...
  const wordInterval = setInterval(() => {
    if (wordIndex < words.length) {
      ...
      clearInterval(wordInterval);
      setTimeout(() => {
        // Simply use the original options array without shuffling
        ...
        // Set correct index based on original array position
        gameState.correctOptionIndex =
          options.indexOf(currentQuestion.answer.correct) + 1;
...
// Remove or comment out the shuffleArray function since it's no longer needed
// VITEZA AFIȘARE TEXT:
...
function endGame(winner) {
  ...
  endGameButtons.style.display = "grid";
}
